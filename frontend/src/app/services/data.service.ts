...
export class DataService {
  ...
  getEquivalences(): Observable<EquivalencesData> {
  ...
  getBenchmark(): Observable<BenchmarkTache[]> {
    return this.benchmark$;
  }

  getDetecteurGaspillage(): Observable<DetecteurGaspillage[]> {
    return this.gaspillage$;
  }

  computeScore(modele: Modele, config: UserConfig): number {
    const total = config.poidsEco + config.poidsPerf + config.poidsPrix;
    if (total === 0) return 0;
    const wEco  = config.poidsEco  / total;
    const wPerf = config.poidsPerf / total;
    const wPrix = config.poidsPrix / total;
    return (modele.score_eco * wEco + modele.score_perf * wPerf + modele.score_prix * wPrix) * 100;
  }
